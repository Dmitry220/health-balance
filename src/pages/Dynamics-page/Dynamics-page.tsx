...
import {currentResultIndexSelector, indicatorsSelector} from '../../Redux/slice/healthIndexSlice'
...
export const DynamicsPage = () => {
    ...
    let titlesIndex = currentResultIndex && Object.keys(currentResultIndex).filter(
        (item) => item !== 'id' && item !== 'date' && item !== 'poll_id'
    )

    const displayGraph = (param: KeysDynamics) => {
        const labelMonths = dynamics && dynamics.map((item) => monthsABBR[new Date(item.date * 1000).getMonth()])
        return {
            labels: labelMonths,
            datasets: [
                {
            ...
            {isLoading ?  <Preloader height={'auto'}/> :
                ...
                >
                    {titlesIndex?.map((item, i) => (
                        <SwiperSlide key={i}>
                            <ChartIndex itemIndex={item as KeysDynamics} displayGraph={displayGraph}/>
                        </SwiperSlide>
                    ))}
                </Swiper>
            </>}
