...
export const BasketPage = () => {
  ...
  const sum = basket.filter(item => item).reduce((sum, current) => sum + current.price, 0)
  ...
  return (
    <div className={'basket-page'}>
      ...
      <div className='basket-page__cards'>
        {
          basket.map(item => (
            <div className='basket-page__card' key={item.id}>
              <BasketCard id={item.id} image={item.image} price={item.price} title={item.title} />
            </div>
          ))
        }
        {
      ...
      <div className='basket-page__button'>
       {ballance < sum && <div className='basket-page__text-danger'>
          На вашем счете недостаточно монет
        </div>}
       {basket.length != 0 && <ShopButton title={'Обменять'} rewardCount={sum} onClick={exchange} />}
      </div>
    </div>
  )
}
