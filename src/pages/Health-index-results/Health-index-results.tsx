...
export const HealthIndexResults = () => {
    ...
    const check = currentResultIndex && (timeConverterUnix(new Date().toLocaleDateString()) - currentResultIndex.date) /
        (24 * 60 * 60) >=
        180.0
               ...
               <div className='health-index-results-page__index'>
                   {
                       indicators?.map((item, index) => (
                           <div className='health-index-results-page__index-item' key={index}>
                               <CardIndex
                                   title={item.title}
                                   value={item.value || 0}
                                   tag={item.tag}
               ...
               <div className='health-index-results-page__disease'>
                   {
                       riskOfDiseases?.map((item, index) => (
                           <div className='health-index-results-page__disease-item' key={index}>
                               <CardDisease
                                   risk={item?.value || 0}
                                   title={item?.title}
                                   tag={item.tag}
