...
export default class ShopService {
  ...
  > {
    return $api.get(`shop-categories?token=${localStorage.getItem('token')}`)
  }
  ...
  static async getOrders(): Promise<AxiosResponse<{ data: IOrders[] }>> {
    return $api.get(`orders?token=${localStorage.getItem('token')}`)
  }

  static async sendOrder(
    products: number[]
  ): Promise<AxiosResponse<{ success: 1 | 0 }>> {
    return $api.post(
