...
export const CardIndex: FC<ICardIndex> = ({ title, value }) => {
  ...
  return (
    <div className={'card-index ' + 'card-index'+(value <=3 ? '_'+value: (value < 18 ? '_'+2 : (value < 25 ? '_'+1 : '_'+3)))}>
      <div className='card-index__head'>
        <div className='card-index__title'>{title}</div>
        <div className={'card-index__hint'}>{hintRisk(value)}</div>
      </div>
      <div className={'card-index__value'+(value <=3 ? '_'+value: (value < 18 ? '_'+2 : (value < 25 ? '_'+1 : '_'+3)))}>{definitionRisk(value)}</div>
    </div>
  )
}
