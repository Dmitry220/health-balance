import React, { ChangeEvent, Dispatch, FC, SetStateAction, useState, forwardRef } from 'react';
    ...
    return (
        <div className={'final-variant'}>
            ...
            <label htmlFor="file" className="final-variant__image">
                {photoPath && <img className={'final-variant__image-main'} src={photoPath} alt="" />}
                ...
                <div className="final-variant__header__info">
                    <div>
                        <select name="" id="" className={definitionColor(type, "final-variant__type")} defaultValue={type}
                            onChange={(e: ChangeEvent<HTMLSelectElement>) => dispatch(setTypeChallenge(+e.target.value))}
                        >
                    ...
                    <div className="final-variant__title">
                        {!isEditTitle && title}
                       {isEditTitle && <input type="text" value={title} 
                        onBlur={()=>setIsEditTitle(false)}
                        onChange={(e: ChangeEvent<HTMLInputElement>) => dispatch(setTitleChallenge(e.target.value))} />}
            ...
            <div className="final-variant__description" >
              {!isEditDescription && description}
                {isEditDescription && <input type="text" value={description} 
                        onBlur={()=>setIsEditDescription(false)}
                        onChange={(e: ChangeEvent<HTMLInputElement>) => dispatch(setDescriptionChallenge(e.target.value))} />}
                <img src={icon_edit} alt="" onClick={()=>setIsEditDescription(true)} />
            </div>
            <div className="final-variant__row">
                ...
                <div className="final-variant__reward">
                    ...
                    />}
                    <img src={icon_edit} alt="" onClick={() => setIsEditReward(prev => !prev)} />
                </div>
            </div>
...
const ExampleCustomInput = forwardRef(({ value, onClick }: any, ref: any) => {
    const type = useAppSelector(typeCreatingChallengeSelector)
    return <div className={definitionColor(type, "final-variant__data")}>
        <img className={'final-variant__data-clock'} src={icon_clock} alt="" />
        {value}
        <img className={'final-variant__data-edit'} src={icon_edit} alt="" onClick={onClick} ref={ref} />
    </div>
});