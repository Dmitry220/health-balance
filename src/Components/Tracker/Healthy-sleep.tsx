...
export const HealthySleep: FC<IHealthySleep> = ({
    ...
    const sleepHours = currentDay?.sleep_time! >= 8
                    ...
                    onClick: async () => {
                        ...
                        if (index === 0) {
                            ...
                            prevTracksSleep.unwrap().then(async (e) => {
                                const dataEveningSunday = e.sleepTrack.find(item => item.type === 4 && item.additional === 'вс')
                                console.log(dataEveningSunday)
                                if (dataEveningSunday && dataEveningSunday?.id != 0) await completeTracks(itemTrackSleep.id, dataEveningSunday?.id)
                            })
