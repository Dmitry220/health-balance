...
export const ModalChat: FC<IModalChat> = ({ active, setActive, idUser }) => {
	...
	const changeCannel = async (channel_id: number, title: string, customers_id: number[]) => {
		customers_id.push(idUser)
		console.log(channel_id, title, customers_id);
		try {
			const response = await ChatService.changeChannel(channel_id, title, customers_id)
	...
	return (
		<div className={active ? "modal-chat active" : "modal"} onClick={() => setActive(false)}>
			<div className={active ? "modal-chat__content active" : "modal-chat__content"} onClick={e => e.stopPropagation()}>
				<div className="modal-chat__row">
					<div className="modal-chat__list-channels channels-list">
						...
						{channels.map(item => <div className="channels-list__item" key={item.id}
							onClick={() => changeCannel(item.id, item.title, item.members.map(member => member.id))}>
				...
				<div className="">
					<input type="text" className='_field'
						placeholder='Название канала'
						style={{ marginBottom: 20 }}
						onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleChannel(e.target.value)} />
					<button className='modal-chat__button _button-white' onClick={newChannel}>Создать новый</button>
				</div>
			</div>
		</div>
	)
