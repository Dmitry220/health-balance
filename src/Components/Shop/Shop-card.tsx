...
import { basketSelector, getProductsCategory, setBasket, shopProductsSelector, deleteBasket, isLoadingSelector } from '../../Redux/slice/shopSlice'
        ...
        products.map(product => (
          <div className={'shop-card'} key={product.id}>
            <div className={'shop-card__container'}>
              <Link to={PRODUCT_SCREEN_ROUTE + '/' + product.id} className='shop-card__img'>
                <img
                  src={
              ...
              <div className='shop-card__row'>
                <Link to={PRODUCT_SCREEN_ROUTE + '/' + product.id} className='shop-card__title'>{product.title}</Link>
                <div className='shop-card__footer'>
                  ...
                  <div className='shop-card__footer-column'>
                    <div className='shop-card__count-product'>{product.quantity} шт.</div>
                    <div className='shop-card__icon-add'>
                      <img src={plus} style={{ transform: basket.find(item=>item.id===product.id) ? 'rotate(45deg)' : 'rotate(0deg)' }} onClick={() => addBasket(product.id, product.image, product.price, product.title)} />
                    </div>
